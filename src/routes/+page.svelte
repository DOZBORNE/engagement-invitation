...
<svelte:head>
	...
	<meta
		name="description"
		content="You are cordially invited to celebrate the engagement of {event.couple
...
<main class="relative z-10">
	...
	>
		<div class="grid flex-1 grid-cols-12 items-center gap-4">
			<div class="col-span-12 sm:col-span-2">
				...
				<div class="mt-1 font-mono text-[0.625rem] tracking-[0.2em] text-[var(--color-rust)]">
					⁂ ENGAGEMENT ⁂
				</div>
			</div>
		...
		>
			<div class="max-w-[20rem]">
				...
				<p class="font-display text-base leading-snug sm:text-lg">
					are pleased to announce their engagement, and would be honoured by your company.
				</p>
			</div>
			<a
				href="#rsvp"
			...
			>
				<span class="border-b border-[var(--color-ink)] pb-0.5">Reply</span>
	...
	<!-- The Date -->
	<section class="border-t border-b border-[var(--color-ink)]/15 px-6 sm:px-12">
		<div class="grid grid-cols-12 items-stretch gap-0 py-12 sm:py-20">
			<div class="col-span-12 flex flex-col justify-center sm:col-span-5">
				...
				<div class="font-display text-7xl leading-none italic sm:text-8xl">
					{event.date.dayNumber}
				</div>
				<div class="mt-2 font-display text-2xl tracking-tight italic sm:text-3xl">
			...
			>
				...
				<div class="font-display text-6xl italic sm:text-7xl">{event.date.timeLabel}</div>
				<p class="mt-6 max-w-md font-display text-lg leading-snug text-[var(--color-ink-soft)]">
					An evening of cocktails, supper, and unhurried conversation, set against the soft hush of
					a Birmingham spring.
				</p>
			</div>
		</div>
	...
	<section class="px-6 sm:px-12">
		<div class="grid grid-cols-12 gap-6 py-16 sm:py-24">
			<div class="col-span-12 sm:col-span-7">
				...
				<div class="font-display text-4xl leading-tight italic sm:text-6xl">
					{event.venue.name}
				</div>
				<div class="mt-6 font-display text-xl text-[var(--color-ink-soft)] sm:text-2xl">
			...
			>
				<div>
					...
					<p class="font-display text-base leading-snug">{event.parking}</p>
				</div>
				<div>
					<div class="eyebrow mb-2">Dress</div>
					<p class="font-display text-base leading-snug">{event.dressCode}</p>
				</div>
			</div>
		</div>
	</section>
	...
	>
		<div class="mx-auto max-w-3xl">
			<div class="text-center">
				...
				<p class="mt-6 font-display text-base text-[var(--color-ink-soft)]">
					{#if deadlinePassed}
						The reply window has closed — but please reach out to us directly.
					{:else}
			...
			{#if submitted || form?.success}
				...
				>
					<div class="eyebrow text-[var(--color-rust)]">Reply received</div>
					...
					<p class="mt-4 font-display text-base text-[var(--color-ink-soft)]">
						{#if (attending === 'yes' || form?.attending) && totalAttending > 0}
							Counting <strong>{form?.guestCount ?? totalAttending}</strong>
							{(form?.guestCount ?? totalAttending) === 1 ? 'seat' : 'seats'} in your name.
						{/if}
					</p>
				</div>
			{:else}
				...
				>
					...
					<fieldset>
						...
						>
							<button
								...
								class="px-4 py-5 text-center transition-colors {attending === 'yes'
									? 'bg-[var(--color-ink)] text-[var(--color-cream)]'
									: 'bg-transparent hover:bg-[var(--color-ink)]/5'}"
							>
								<div class="font-mono text-[0.625rem] tracking-[0.22em] uppercase opacity-70">
									Joyfully
								</div>
								<div class="mt-1 font-display text-2xl italic sm:text-3xl">accepts</div>
							</button>
							<button
								type="button"
								onclick={() => (attending = 'no')}
								class="px-4 py-5 text-center transition-colors {attending === 'no'
									? 'bg-[var(--color-ink)] text-[var(--color-cream)]'
									: 'bg-transparent hover:bg-[var(--color-ink)]/5'}"
							>
								<div class="font-mono text-[0.625rem] tracking-[0.22em] uppercase opacity-70">
								...
								<div class="mt-1 font-display text-2xl italic sm:text-3xl">declines</div>
							</button>
						</div>
					</fieldset>
					...
					{#if attending === 'yes'}
						...
						<fieldset>
							<div class="mb-4 flex items-baseline justify-between">
								...
								<span class="font-mono text-xs tracking-[0.18em] uppercase text-[var(--color-clay)]">
									{guests.length}
							...
							<div class="space-y-6">
								{#each guests as guest, i (i)}
									<div class="border-t border-[var(--color-ink)]/15 pt-6">
						...
						<!-- Note -->
						<fieldset>
							<legend class="eyebrow mb-3">A note for the couple <span class="text-[var(--color-clay)]">(optional)</span></legend>
							<textarea
								bind:value={note}
								rows="3"
					...
					<div class="border-t border-[var(--color-ink)]/15 pt-8">
						...
						>
							{submitting ? 'Sending…' : deadlinePassed ? 'Replies closed' : 'Send reply'}
						</button>
					</div>
				</form>
			{/if}
	...
	<footer class="px-6 py-10 sm:px-12">
		...
		<div class="flex flex-wrap items-center justify-between gap-3">
			<span class="eyebrow">Set in Fraunces · Birmingham · {event.romanYear}</span>
			<span class="eyebrow">{event.couple.first} &amp; {event.couple.second}</span>
		</div>
	</footer>
</main>
